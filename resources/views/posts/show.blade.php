            ...
            @auth
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{route('posts.destroy', $post)}}" method="post">
                        @method('DELETE')
                        @csrf
                        <input type="submit" value="Eliminar publicación" class="bg-red-500 hover:bg-red-600 p-2 rounded text-white font-bold cursor-pointer mt-4">
                    </form>
                @endif
            @endauth
            ...
            <div class="shadow bg-white p-5 mb-5">
                ...
                <form action="{{route('comentarios.store', ['post' => $post, 'user' => $user])}}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea id="comentario" name="comentario" class="border p-3 w-full rounder-lg @error('comentario')
                        border-red-500 @enderror" placeholder="Agrega un comentario"></textarea>
                        @error('comentario')
